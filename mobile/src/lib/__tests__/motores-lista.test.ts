...
describe('a fusão: o que o app conhece, mais o que o servidor aprovou', () => {
  ...
  it('a variante aprovada é selecionável; sem a lista, ela nem existe', () => {
    const conhecidos = motoresDoRecurso('saude-do-sono', { motores: [ACME], lidaEm: 1 });
    ...
    expect(motoresDoRecurso('saude-do-sono', null).some((m) => m.id === ACME.motor)).toBe(false);
  });
  ...
  it('o motivo de bloqueio exige o catálogo — o estático não conhece a variante', () => {
    ...
    expect(motivoDeBloqueio(SAUDE, ACME.motor, motoresDoRecurso('saude-do-sono', { motores: [ACME], lidaEm: 1 }))).toBeNull();
  });
});
