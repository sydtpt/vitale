 ...
 * 27/08/2026 ela vivia aqui e, quase idêntica, em `web/.../data/overview.ts`:
 * 98,5% do código era o mesmo, e a única diferença real era esta — de onde vêm
 * o rótulo e a cor de um `activityId`. Essa diferença virou o parâmetro
 * `metaOf`, e é tudo o que sobrou deste arquivo.
 *
 * A assinatura posicional foi preservada de propósito: os call sites e os testes
 * continuam valendo palavra por palavra, o que é o que torna a mudança
 * verificável. Quem chama daqui não precisa saber que a conta mudou de casa.
 */
...
const metaOf: ActivityMetaLookup = (activityId) => ({
  label: getActivityMeta(activityId).label,
