...
import { diffDays, gridMonthStarts, parseLocal, planFor, yearGrid } from '../period/bucket-plan';
...
/**
 * As 12 colunas onde os meses começam — reexporta a regra da grade, para a tela
 * não precisar conhecer `period/bucket-plan`.
 */
export function yearHeatmapMonthStarts(weeks: RegistroHeatCell[][]): HeatmapMonthStart[] {
  return gridMonthStarts(weeks);
}

/** A grade do ano com a marca de cada dia pintada por cima (ver `yearGrid`). */
export function yearHeatmap(dates: string[], year: number): RegistroHeatCell[][] {
  ...
  return yearGrid(year).map((week) =>
    week.map((c) => ({ date: c.date, marked: marked.has(c.date), inYear: c.inYear })),
  );
}
