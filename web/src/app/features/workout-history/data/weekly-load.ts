 ...
 * de carga forte crescente. Os buckets são estruturalmente compatíveis com
 * `OverviewBucket` → entram direto no `StackedBarChartComponent`.
 */
...
export interface WeeklyLoad {
  buckets: WeekLoadBucket[];   // N buckets, ordem cronológica (antiga → atual)
...
export function buildWeeklyLoad(
  ...
  for (const a of activities) {
    const z = a.hrZones;
    if (!z) continue;
    const monday = mondayOf(new Date(a.startAt));
  ...
  const current = perWeek.get(keyOf(currentMonday));
  ...
  const baseline = priorHard.length
    ? priorHard.reduce((a, b) => a + b, 0) / priorHard.length
    : 0;
  const highLoadAlert =
    priorHard.length >= MIN_BASELINE_WEEKS && hardS > baseline * HIGH_LOAD_FACTOR;
