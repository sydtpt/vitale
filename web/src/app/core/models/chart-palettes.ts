/**
 * Paletas de cor para os gráficos (web). Espelha
 * mobile/src/lib/chart-palettes.ts — mantenha os dois em sincronia.
 *
 * Cada paleta remapeia as cores-base por tipo de atividade (tokens `T` usados em
 * @core/models/activity-types) para um conjunto harmônico com a paleta creme do
 * app. A cor segue a ENTIDADE (tipo de atividade), nunca o ranking.
 *
 * Trio principal (laranja/verde/azul = Corrida/Yoga/Ciclismo) validado com o
 * script de dataviz sobre o fundo creme (banda de luminosidade, chroma e separação
 * de visão normal ≥15). CVD na faixa de aviso é aceitável com a legenda rotulada.
 */

// Cores-base (T do shared, ver activity-types) — chaves do remap.
const BASE = {
  orange: '#F25C2B', // T.primary
  blue: '#6E8CC9', // T.blue
  green: '#6FA86A', // T.green
  yellow: '#F5B946', // T.yellow
  rose: '#E26A8A', // T.rose
  brown: '#B4825B', // T.casa
  deep: '#D9491B', // T.primaryDeep
  ink: '#1F1B16', // T.ink
} as const;
...
/** Converte uma cor-base (hex dos tokens de atividade) para a cor da paleta ativa.
 * Cores fora do mapa (ex.: zonas de FC) passam sem alteração. */
...
/** Amostra representativa para prévia (laranja, verde, azul, rosa, amarelo). */
