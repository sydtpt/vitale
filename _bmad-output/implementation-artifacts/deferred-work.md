...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-o-prompt-aprende-as-regras-novas.md`
  summary: A quinta regra trata uma base NEGADA como nomeação dela. "Sem a normal do período, o único contraste é 862" reprova por inversão, com 862 sendo valor de B1 — mas a frase não atribuiu 862 a B3, disse que B3 não existe. O conserto é excluir a marca negada ("sem X", "não há X", "X: não foi medida") da nomeação, e o argumento é que isso NÃO afrouxa a regra: uma base negada não tem valor por construção, logo nenhum número do pacote é valor dela, logo a exclusão não pode mascarar inversão nenhuma. O caso não fica sem conferência — cai na absolvição por parágrafo e reprova por AUSÊNCIA, que é a acusação certa. Com isso a declaração de ausência pode voltar para a linha do fato e o bloco por caderno da 1.5 some, deixando o código mais simples do que ficou.
  evidence: Achado com execução por duas camadas de revisão independentes na Story 1.5 e reproduzido à mão. Ficou fora da 1.5 porque mexe na severidade da quinta regra, que o *Never* daquela spec proíbe — o dono decidiu em 09/09 fazer logo depois de a 1.5 mesclar e ANTES da Story 1.15, que é onde ele lê edições impressas e uma recusa injusta desperdiça o tempo dele. Requer provas negativas nos dois sentidos e o detector de negação colado à marca.
- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-o-prompt-aprende-as-regras-novas.md`
