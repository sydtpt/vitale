...
- **Ausência tem gramática:** métrica que começou a ser registrada depois do fim do período entra como "não medida", nunca como zero. O marco sai do dado (criação do hábito ou do registro, primeiro dia da métrica de saúde), nunca de uma data escrita no código. Zero continua sendo medida quando o registro já existia. Caderno com tudo "não medido" é vazio e some. **Implementado na 2.6 (23/09/2026):** o critério é de dois tipos — *medição* (saúde, passos, andares) não tem medida quando nenhum dia do período carregou valor; *contagem de ato* (hábito, registro, tarefa, compra, atividade, esporte) é zero legítimo quando o registro já existia e não medida quando ainda não existia. O canal é `atual: null` / `Base.valor: null`, sem campo novo; `PACOTE_VERSAO` foi a 4.
...
- **Decisões do dono antes do código:**
  - 2.6: **DECIDIDA em 23/09/2026 — setembro e outubro de 2023 serão REIMPRESSAS.** Elas foram impressas antes da 2.6 e carregam o zero no lugar da ausência ("água, café, cerveja e smoke somaram 0 dias", "passos por dia: 0"). **Antes de reimprimir**, o texto atual das duas vai para `docs/specs/revista-retrospectiva/`, como se fez com as 7 edições antigas na janela da 1.9 — quem exporta é o dono, com o JWT dele. É **pré-condição da 2.3**, não da 2.6: a 2.6 não reimprime nem apaga edição nenhuma.
