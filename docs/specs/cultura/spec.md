...
- Chave de provedor não pode ir no bundle do cliente. O TMDB é o único provedor com chave, e guardá-la exige edge function Deno, no padrão de `strava-oauth`. **É esta constraint que impede a CAP-1 de ser feita inteira no cliente** — junto do CORS e do rate-limit do MusicBrainz.
